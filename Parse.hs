...
parse p s
  | length (apply (p <* endOfInput) s) == 0 = Left "Sirul de intrare nu a fost complet consumat sau parsare ambigua"
  | otherwise = Right (fst $ head $ apply (p <* endOfInput) s)

instance Monad Parser where
    pa >>= k = Parser (\input -> [(b, restb) | (a, resta) <- apply pa input, (b, restb) <- apply (k a) resta])

cifraSemn :: Parser Int
cifraSemn = (char '+' *> pure 1) <|> (char '-' *> pure (-1))
